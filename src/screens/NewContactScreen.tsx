...
import { validateCountryCode, validateFirstName, validatePhoneNo } from "../util/Validation";
import { ALERT_TYPE, Toast } from "react-native-alert-notification";
import { useSendNewContact } from "../socket/UseSendNewContact";

type NewContacScreenProp = NativeStackNavigationProp<
  RootStack,
  "NewContactScreen"
>;

export default function NewContactScreen() {
  const navigation = useNavigation<NewContacScreenProp>();
      ...
      <View className="flex-1 px-5">
        <View className="flex-row items-center gap-x-2 h-14">
          ...
          <View className="flex-1 h-14">
            <FloatingLabelInput label="First Name" className="" value={firstName} onChangeText={(text)=>setFirstname(text)}/>
          </View>
        </View>
        <View className="flex-row items-center gap-x-2 mt-8 h-14">
          <Feather name="user" size={24} color="black" />
          <View className="flex-1 h-14">
            <FloatingLabelInput label="Last Name" className="" value={lastName} onChangeText={(text)=>setLastname(text)}/>
          </View>
        </View>
        <View className="border-b-2 border-b-sky-800 justify-center items-center flex-row h-14 mb-3">
          <CountryPicker
        ...
        <View className="flex-row items-center gap-x-2 mt-8 h-14">
          ...
          <View className="flex-1 h-14">
            <FloatingLabelInput label="Phone" className="" value={phoneNo} onChangeText={(text)=>setPhoneNo(text)}/>
          </View>
        </View>
        <View className="mt-10">
          <Pressable className="bg-green-600 h-14 items-center justify-center rounded-full" onPress={()=>{
            const firstNameValid = validateFirstName(firstName);
            const lastNameValid = validateFirstName(lastName);
